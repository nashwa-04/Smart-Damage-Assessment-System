...
<head>
    ...
    <title>تأكيد البريد الإلكتروني - نظام تقييم الأضرار</title>
        ...
        <div class="w-full max-w-md">
            <div class="glass rounded-3xl p-6 sm:p-8 lg:p-10 shadow-2xl">
                <div class="text-center mb-6">
                    ...
                    <h2 class="text-2xl sm:text-3xl font-black text-charcoal mb-2">{{ __('Verify Email') }}</h2>
                    <p class="text-charcoal/70 text-sm leading-relaxed">{{ __('Thanks for signing up! Before getting started, could you verify your email address by clicking on the link we just emailed to you? If you didn\'t receive the email, we will gladly send you another.') }}</p>
                </div>

                @if (session('status') == 'verification-link-sent')
                    <div class="mb-5 p-3 bg-sage/10 border border-sage/30 rounded-xl text-sage text-sm font-medium">
                ...
                <div class="space-y-4">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full text-sm text-charcoal/60 hover:text-charcoal transition-colors font-medium rounded-xl">
                            {{ __('Log Out') }}
                        </button>
                    </form>
                </div>
